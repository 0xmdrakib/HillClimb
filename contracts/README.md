...
- New tokens use immutable, content-addressed URLs from the project's paid Lighthouse gateway for `tokenURI` and the primary image. Metadata also retains the canonical `ipfs://<image CID>` in `properties.files`.
